    ...
    signin({dispatch, commit, rootState}, body) {
      ...
      axios.post('/signin', body).then(res => {
        ...
        if(res.data.done && res.data.confirmed===1) {
            ...
            // alert(res.data.type);
            else if(res.data.type==='exams') router.replace('/exam');
            else if(res.data.type==='examsOnly') router.replace('/examsOnly');
            else if(res.data.type==='fullCourse') router.replace('/fullCourse');
