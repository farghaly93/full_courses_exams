        ...
        <div class="row">
            ...
            <div class="chatt col-md-9">
                <section class="examToCorrect" v-if="isStudentExam">
                    ...
                    <div class='row'>
                        <div class="col-md-6"><button @click="submitCorrection" type="button" class="btn btn-primary">Submit correction</button></div>
                        <div class="col-md-6"><button @click="()=>removeCorrection(exam._id, exam.username, exam.examId)" type="button" class="btn btn-danger">Delete</button></div>
                    </div>
                    <v-divider/>
                </section>
                <section class="studentExams" v-if="!isStudentExam">
                    <div style="color:#555;font-size:22px;font-weight:bold;" class="text-center">{{username}}</div>
                     <div
                        ...
                        :key="i"
                        >
                        <p style="color:#fff;font-size:17px;font-weight:bolder;">({{exam.number}}) اجابة امتحان رقم </p>
                    </div>
                </section>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        removeCorrection(id, username, examId) {
              axios.delete('/removeCorrection/'+id+'/'+examId+'/'+this.userId).then(res => {
                  if(res.data.done) {
                      this.$store.dispatch('writemessage', 'Correction saved successfully...');
                      this.getExams(id, username);
                  } else {
                      this.$store.dispatch('writemessage', 'حدثت مشكلة اثناء الحذف..');
                  }
              })
        }
    },
    watch: {
        stage(val) {
            axios.get('/getUsersExam/'+val+'/exams').then(res => {
